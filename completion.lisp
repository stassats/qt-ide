...
(defclass completer (list-widget)
  ...
  (:default-initargs :description #'print-string-cased))
...
(defmethod key-press-event ((widget completer) event)
  (let* ((key (#_key event))
         ...
         (space-p (= key (enum-value (#_Qt::Key_Space)))))
    (cond ((= key (enum-value (#_Qt::Key_Escape)))
           (#_close widget)
           (#_setFocus parent))
          ((or space-p
          ...
          ((or (= key (enum-value (#_Qt::Key_Up)))
               (= key (enum-value (#_Qt::Key_Down)))
               (= key (enum-value (#_Qt::Key_Left)))
               (= key (enum-value (#_Qt::Key_Right)))
               (= key (enum-value (#_Qt::Key_PageUp)))
               (= key (enum-value (#_Qt::Key_PageDown)))
               (= key (enum-value (#_Qt::Key_Home)))
               (= key (enum-value (#_Qt::Key_End))))
           (stop-overriding))
          (t
